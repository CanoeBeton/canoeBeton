...
const Sponsor: FunctionComponent<PropsWithChildren> = ({}) => {
  ...
  const {data: partenaire , status: partenaireStatus} = useQuery({ queryKey: ['partenaire'], queryFn: () => getPartenaire(true) })
  const {data: year, status: yearStatus} = useQuery({queryKey:['active year'], queryFn: () => getActiveYear()})

  return (
    <div className={styles.page}>
      <div className={styles.h1}>Partenaires</div>
      <h2>Nos partenaire de l&apos;année! </h2>
      {yearStatus == 'error' && <span>Une erreur est survenue, veillez réessayer plustard</span>}
      ...
      {partenaireStatus == 'error' && <span>Une erreur est survenue, veillez réessayer plustard</span>}
