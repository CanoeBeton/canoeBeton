...
const Header: FunctionComponent<PropsWithChildren<HeaderProps>> = ({
  ...
  return (
    <div
      className={
        'flex z-10 justify-between h-10 relative rounded-md m-2.5 shadow-header bg-beton'
      }
    >
      <img src="/favicon.webp" alt="Logo" />
      <div
        className={
          'flex flex-grow gap-6 items-center justify-center pr-2 max-sm:hidden'
        }
      >
        {MenuOptions()}
      </div>
      ...
      <div
        className={`flex flex-col fixed top-0 right-[50vw] bg-white w-[50vw] 
                items-center h-full justify-around transition-transform duration-500 transform ${
                  isMenuOpen ? 'translate-x-[50vw]' : 'translate-x-[100vw]'
                }
